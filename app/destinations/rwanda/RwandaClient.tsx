...
interface RwandaClientProps {
  highlights: Highlight[]
  tours: Tour[]
}

export default function RwandaClient({ highlights, tours }: RwandaClientProps) {
  return (
    <div>
      {/* Overview */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
              ...
              <p className="text-lg text-gray-600 mb-6">
                Rwanda, the "Land of a Thousand Hills," offers unique wildlife experiences like gorilla trekking and is now one of Africa's top eco-tourism destinations.
              </p>
              <p className="text-lg text-gray-600 mb-8">
                From the misty Volcanoes National Park to the ancient Nyungwe forests, Rwanda delivers intimate encounters and breathtaking landscapes.
              </p>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
              <img
                src="https://ik.imagekit.io/jinx/travel/Webp.net-resizeimage.jpg?updatedAt=1750019108294"
                alt="Rwanda Gorillas"
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {highlights.map((highlight, index) => (
              <motion.div key={highlight.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: index * 0.1 }} viewport={{ once: true }}>
                <Card className="overflow-hidden hover:shadow-xl transition-shadow duration-300">
                  <div className="relative overflow-hidden">
                    <img src={highlight.image} alt={highlight.name} className="w-full h-48 object-cover hover:scale-105 transition-transform duration-300" />
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {tours.map((tour, index) => (
              <motion.div key={tour.id} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: index * 0.1 }} viewport={{ once: true }}>
                <Card className="overflow-hidden hover:shadow-xl transition-shadow duration-300">
                  <div className="relative overflow-hidden">
                    <img src={tour.image} alt={tour.title} className="w-full h-48 object-cover hover:scale-105 transition-transform duration-300" />
